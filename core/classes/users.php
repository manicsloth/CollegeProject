...
class Users{
	...
	//To protect areas that non logged in users are restricted from. redirects to login page
	...
	//function to create new member accounts
		...
		//email user activation link using Swift Mailer (emails to my own email instead of users for development purposes)
		require_once '../swift/lib/swift_required.php';
		$transport = Swift_MailTransport::newInstance();
		$mailer = Swift_Mailer::newInstance($transport);
		$message = Swift_Message::newInstance('Thankyou for Registering to Walk Kernow') ->setFrom(array('webkrunch@gmx.co.uk' => 'Walk_Kernow')) ->setTo(array('ab122701@ghs.truropenwith.ac.uk')) ->setBody("Hello " . $email. ",\r\nThank you for registering with us. Please visit the link below so we can activate your account:\r\n\r\nhttp://ns6577.hostgator.com/~manic/activate.php?email=" . $email . "&email_code=" . $email_code . "\r\n\r\n-- Walk Kernow)");
	...
	public function login_credentials($email, $password) {
	 //function to check if login credentials are correct. returns ID of the account that the provided email is connected to if credentials are valid.
		global $bcrypt; 
